...
public abstract class ConnectivityIntentCommand extends AbstractShellCommand {
    ...
    @Option(name = "--encryption", description = "Encryption",
            required = false, multiValued = false)
    private boolean encryption = false;

    @Option(name = "--latencySensitive", description = "Latency sensitive",
            required = false, multiValued = false)
    private boolean latencySensitive = false;

    @Option(name = "--appType", description = "Application type",
            required = false, multiValued = false)
    private String applicationString = null;

    @Option(name = "-a", aliases = "--availability", description = "Availability in percentage",
            required = false, multiValued = false)
    private String availabilityString = null;

    @Option(name = "--ha",  description = "High Availability",
            required = false, multiValued = false)
    ...
    protected List<Constraint> buildConstraints() {
        ...
        // Check for encryption constraint
        if(encryption) {
            constraints.add(new EncryptionConstraint());
        }

        // Check for latency sensitive constraint
        if(latencySensitive) {
            constraints.add(new LatencySensitive());
        }

        if (!isNullOrEmpty(availabilityString)) {
            double parseAvailability = Double.parseDouble(availabilityString);
            final AvailabilityConstraint availabilityConstraint = new AvailabilityConstraint(parseAvailability);
            constraints.add(availabilityConstraint);
        }

        // Check for restoration constraint
        if (!isNullOrEmpty(applicationString)) {
            final ApplicationType applicationType = ApplicationType.valueOf(applicationString);
            constraints.add(new ApplicationConstraint(applicationType));
        }
        ...
        if(highAvailability){
            constraints.add(new HighAvailabilityConstraint());
        }
