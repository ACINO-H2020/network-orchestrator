...
@Component
public abstract class DomainIntentInstallerBase {

    final Logger log = getLogger(IntentManager.class);
    ...
    void applyInternal(IntentOperationContext<? extends DomainIntent> context) {
        Optional<IntentData> toUninstall = context.toUninstall();
        Optional<IntentData> toInstall = context.toInstall();

        List<? extends DomainIntent> uninstallIntents =
                context.intentsToUninstall();
        List<? extends DomainIntent> installIntents =
                context.intentsToInstall();
        ...
        if (toUninstall.isPresent()) {
            ...
            trackerService.removeTrackedResources(intentData.key(),
                                                  intentData.intent()
                                                          .resources());
            uninstallIntents.forEach(installable -> trackerService.removeTrackedResources(
                    intentData.intent().key(),
        ...
        if (toInstall.isPresent()) {
            ...
            trackerService.addTrackedResources(intentData.key(),
                                               intentData.intent().resources());
            installIntents.forEach(installable -> trackerService.addTrackedResources(
                    intentData.key(),
                    installable.resources()));
        }

        // Generate domain Intent operations
        DomainIntentOperations.Builder builder =
