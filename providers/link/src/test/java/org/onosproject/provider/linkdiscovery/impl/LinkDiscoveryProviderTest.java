...
public class LinkDiscoveryProviderTest {
    ...
    private static final String ANN_KEY_1 = "TEST1";
    private static final String ANN_VALUE_1 = "ASD1";
    private static final String ANN_KEY_2 = "TEST2";
    private static final String ANN_VALUE_2 = "ASD2";
    private static final LinkDescription LINK1 = new DefaultLinkDescription(CP11, CP21,
                                                                            DIRECT,
            DefaultAnnotations.builder().putAll(LINK_ANNOTATIONS).set(ANN_KEY_2, ANN_VALUE_2).build());

    private static final LinkDescription LINK1_ANNOTATED = new DefaultLinkDescription(CP11, CP21,
            DIRECT,
            DefaultAnnotations.builder().set(ANN_KEY_1, ANN_VALUE_1).build());
    ...
    private final Device device1 = new MockDevice(provider.id(), DEVICE_ID_1, SWITCH,
    ...
    public void activate() throws Exception {
        ...
        assertFalse("Executor should be running", provider.scheduledExecutorService.isShutdown());
        assertFalse("Executor should be running", provider.scheduledExecutorService.isTerminated());
    ...
    public void deactivate() throws Exception {
        ...
        assertTrue(provider.scheduledExecutorService.isShutdown());
    ...
    public void linksTestForStoredDevice() {
        ...
        assertAfter(1100, () -> {
            ...
            assertFalse("Link2 should be removed",
                    providerService.discoveredLinkDescriptions().containsKey(LINKKEY2));
            assertTrue("Link1 should be present",
                    providerService.discoveredLinkDescriptions().containsKey(LINKKEY1));
            assertEquals("Link1 should contain the original annotation", ANN_VALUE_2,
                    providerService.discoveredLinkDescriptions()
                            .get(LINKKEY1).annotations().value(ANN_KEY_2));
            assertTrue("Link3 should be present",
    ...
    @Test
    public void linksAnnotationTest() {
        provider.modified(CONTEXT);
        providerService.discoveredLinkDescriptions().put(LINKKEY1, LINK1);
        providerService.discoveredLinkDescriptions().put(LINKKEY2, LINK2);
        testLink.addLinkDesc(LINK1_ANNOTATED);
        testLink.addLinkDesc(LINK2);
        assertAfter(1100, () -> {
            assertEquals("Link1 should be annotated", SCHEME_NAME.toUpperCase(),
                         providerService.discoveredLinkDescriptions()
                    .get(LINKKEY1).annotations().value(AnnotationKeys.PROTOCOL));
            assertTrue("Link1 should be annotated with both annotations", ANN_VALUE_1 ==
                    providerService.discoveredLinkDescriptions()
                            .get(LINKKEY1).annotations().value(ANN_KEY_1) && ANN_VALUE_2 ==
                    providerService.discoveredLinkDescriptions()
                            .get(LINKKEY1).annotations().value(ANN_KEY_2));
            assertEquals("Link2 should be annotated", SCHEME_NAME.toUpperCase(),
                    providerService.discoveredLinkDescriptions()
                            .get(LINKKEY2).annotations().value(AnnotationKeys.PROTOCOL));
