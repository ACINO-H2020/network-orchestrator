...
public abstract class TopologySimulator {
    ...
    public void createRoadmDevice(DeviceId id, int chassisId, Device.Type type, int portCount) {
        DeviceDescription desc =
                new DefaultDeviceDescription(id.uri(), type,
                        "WSS", "0.1", "0.1", "1234",
                        new ChassisId(chassisId));
        deviceProviderService.deviceConnected(id, desc);
        deviceProviderService.updatePorts(id, buildRoadmPorts(portCount));
    }

    public void createOTNDevice(DeviceId id, int chassisId, Device.Type type, int portCount, int portspeed) {

        DeviceDescription desc =

                new DefaultDeviceDescription(id.uri(), type,
                        "Transponder", "0.1", "0.1", "1234",
                        new ChassisId(chassisId));
        deviceProviderService.deviceConnected(id, desc);
        if(portspeed == 1)
            deviceProviderService.updatePorts(id, build1GBEOTNPorts(portCount));
        else if(portspeed == 10)
            deviceProviderService.updatePorts(id, build10GBEOTNPorts(portCount));
        else if(portspeed == 100)
            deviceProviderService.updatePorts(id, build100GBEOTNPorts(portCount));
        else
            deviceProviderService.updatePorts(id, build10GBEOTNPorts(portCount));
    }
    ...
    public void createLink(ConnectPoint one, ConnectPoint two, Link.Type type, boolean isBidirectional) {
        linkProviderService.linkDetected(new DefaultLinkDescription(one, two, type,true));
        if (isBidirectional) {
            linkProviderService.linkDetected(new DefaultLinkDescription(two, one, type,true));
        }
        int iterate = 1000;
        // Block until the link is actually created
        Link newLink = null;
        while (newLink == null && iterate > 0) {
            newLink = linkService.getLink(one, two);
            try {
                Thread.sleep(10);
        ...
        if (newLink == null) {
            log.error("The new link did not appear after 10 seconds! Aborting!");
        }
    }
    public void createLinkAnnotated(ConnectPoint one, ConnectPoint two, Link.Type type, boolean isBidirectional,SparseAnnotations... annotations) {
        linkProviderService.linkDetected(new DefaultLinkDescription(one, two, type,true,annotations));
        if (isBidirectional) {
            linkProviderService.linkDetected(new DefaultLinkDescription(two, one, type,true,annotations));
        }
        int iterate = 1000;
        // Block until the link is actually created
        Link newLink = null;
     ...
     */
    // TODO: assumes Gigabit Ethernet everywhere..
    protected List<PortDescription>
    buildPorts(int portCount) {
        List<PortDescription> ports = Lists.newArrayList();
        for (int i = 1; i <= portCount; i++) {
            ports.add(new DefaultPortDescription(PortNumber.portNumber(i), true,
                    Port.Type.COPPER, 10000));
        }
        return ports;
    }

    protected List<PortDescription>
    buildRoadmPorts(int portCount) {
        List<PortDescription> ports = Lists.newArrayList();
        for (int i = 1; i <= portCount; i++) {
            ports.add(omsPortDescription(PortNumber.portNumber(i),
                    true, // enabled
                    Frequency.ofTHz(184.50), // minFreq
                    Frequency.ofTHz(193.10), // maxFreq
                    Frequency.ofGHz(50.0) // grid spacing
                    ));
        }
        return ports;
    }
    // TODO: add numLambdas as a variable to null-create-roadm!
    protected List<PortDescription>
    build100GBEOTNPorts(int portCount) {
        // totally 3 lambdas in the network ..
        final int numLambdas = 3;
        List<PortDescription> ports = Lists.newArrayList();
        // TODO: how should number of ports of each kind, and lambda assignment be done?
        // This approach is quite ugly..
        for (int i = 1; i <= portCount; i++) {
            if (i % 2 == 0) {
                ports.add(ochPortDescription(PortNumber.portNumber(i),
                        true, // enabled
                        OduSignalType.ODU0,
                        true, // tunable
                        OchSignal.newDwdmSlot(ChannelSpacing.CHL_50GHZ, OTNCount%numLambdas))
                );
                OTNCount++;
            } else {
                ports.add(oduCltPortDescription(PortNumber.portNumber(i),
                        true, // enabled
    ...
    // TODO: add numLambdas as a variable to null-create-roadm!
    protected List<PortDescription>
    build10GBEOTNPorts(int portCount) {
        // totally 3 lambdas in the network ..
        final int numLambdas = 3;
        List<PortDescription> ports = Lists.newArrayList();
        // TODO: how should number of ports of each kind, and lambda assignment be done?
    ...
    protected List<PortDescription>
    build1GBEOTNPorts(int portCount) {
        // totally 3 lambdas in the network ..
        final int numLambdas = 3;
        List<PortDescription> ports = Lists.newArrayList();
        // TODO: how should number of ports of each kind, and lambda assignment be done?
