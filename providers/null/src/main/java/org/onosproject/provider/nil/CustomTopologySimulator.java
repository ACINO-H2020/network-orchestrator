...
public class CustomTopologySimulator extends TopologySimulator {
    ...
    public void createRoadmDevice(DeviceId id, String name, Device.Type type, int portCount) {
        int chassisId = (int) fromHex(id.uri().getSchemeSpecificPart());
        createRoadmDevice(id, chassisId, type, portCount);
        nameToId.put(name, id);
    }
    public void createOTNDevice(DeviceId id, String name, Device.Type type, int portCount, int portspeed) {
        int chassisId = (int) fromHex(id.uri().getSchemeSpecificPart());
        createOTNDevice(id, chassisId, type, portCount, portspeed);
        nameToId.put(name, id);
    }

    public void configureTransponder(DeviceId dev, List<PortDescription> portDescriptions){
        deviceProviderService.updatePorts(dev, portDescriptions);
    }
    /**
